document.addEventListener("DOMContentLoaded", () => {
    ...
    contactForms.forEach(form => {
        form.addEventListener("submit", (e) => {
            ...
            if (isValid) {
                ...
                const pageName = document.title || "Trustynets Website";

                // Build WhatsApp message
                const waMessage = `Hi Trustynets,%0A%0AI'd like to request a quote.%0A%0A*Name:* ${encodeURIComponent(name)}%0A*Phone:* ${encodeURIComponent(phone)}%0A*City:* ${encodeURIComponent(city)}%0A*Service:* ${encodeURIComponent(service)}%0A*Submitted from:* ${encodeURIComponent(pageName)}%0A%0APlease get back to me. Thank you!`;
                const waLink = `https://wa.me/919090795564?text=${waMessage}`;

                // Build Email mailto link
                const emailSubject = encodeURIComponent(`Quote Request – ${service} in ${city}`);
                const emailBody = encodeURIComponent(`Hello Trustynets,\n\nI'd like to request a quote for your services.\n\nName: ${name}\nPhone: ${phone}\nCity: ${city}\nService: ${service}\nSubmitted from: ${pageName}\n\nPlease get back to me at your earliest convenience.\n\nThank you!`);
    ...
    const defaultMsg = "Hi Trustynets, I'd like to get in touch regarding safety netting installation. Please assist me.";
    const waLinks = document.querySelectorAll("a[href*='wa.me/919090795564']");
    ...
    const cityContacts = {
        "Bangalore": { phone: "+91 90907 95564", whatsapp: "919090795564" },
        "Hyderabad": { phone: "+91 90907 95564", whatsapp: "919090795564" },
        "Vizag": { phone: "+91 90907 95564", whatsapp: "919090795564" },
        "Bhubaneswar": { phone: "+91 90907 95564", whatsapp: "919090795564" }
    };
    ...
    /* ==========================================================================
       ...
       ========================================================================== */
    // 1. Dynamic injection of Gooey Blobs markup for all WhatsApp & Call links
    const wpCallLinks = document.querySelectorAll('.btn-wp, a[href*="wa.me"], a[href*="tel:"]');
    wpCallLinks.forEach(link => {
        // Exclude the mobile sticky bottom bar items (already have markup) and floating circular cta icons
        if (link.classList.contains('mobile-sticky-item') || link.classList.contains('floating-btn')) return;
        
        // Add styling classes
        link.classList.add('btn-gooey-dynamic');
        if (link.classList.contains('btn-wp') || link.href.includes('wa.me')) {
            link.classList.add('btn-gooey-dynamic-wp');
        } else {
            link.classList.add('btn-gooey-dynamic-call');
        }
        
        // Prepend gooey blobs container if not already present
        if (!link.querySelector('.gooey-bg')) {
            const gooeyBg = document.createElement('div');
            gooeyBg.className = 'gooey-bg';
            gooeyBg.innerHTML = '<div class="blob"></div><div class="blob"></div><div class="blob"></div>';
            link.prepend(gooeyBg);
        }
        
        // Wrap any text nodes in <span> to ensure z-indexing layer works correctly
    ...
    // 2. Attach touchstart/mousedown animations to both mobile-sticky and dynamic buttons
    const allGooeyButtons = document.querySelectorAll(".mobile-sticky-item, .btn-gooey-dynamic");
